...
hours_to_map = [dt.datetime(2022, 1, 1, hour) for hour in range(23,24)] # monday 10th january 2022
...
cluster = 108
resolution = 100 # Resolution of the map in meters
...
def interpolate_points(x, variable, lats, lons):
    # Extraure latituds, longituds i la variable a interpolar
    x = x.reset_index()[['Latitude', 'Longitude', variable]]

    # Separa les coordenades i els valors de la variable
    lon_coords = x['Longitude'].values
    lat_coords = x['Latitude'].values
    var_vals = x[variable].values

    # Crear una malla de latituds i longituds per a la interpolació
    lats, lons = np.meshgrid(lats, lons)

    # Preparar els punts per a la interpolació
    points = np.column_stack((lon_coords, lat_coords))

    # Interpolació lineal amb 'griddata'
    interpolated_vals = RectBivariateSpline(lon_coords, lat_coords, var_vals)

    # Aplanar els resultats de la malla
    interpolated_coords = np.vstack([lats.ravel(), lons.ravel()]).T
    interpolated_results = np.hstack([interpolated_coords, interpolated_vals.ravel()[:, np.newaxis]])

    # Combinar els resultats en un DataFrame
    return pd.DataFrame(interpolated_results, columns=['Latitude', 'Longitude', f'{variable}_interp'])
...
#     proj_wgs84 = Proj(init='epsg:4326')  # Sistema de coordenades WGS84 (geogràfic)
#     proj_utm = Proj(init='epsg:25831')   # Sistema UTM per la zona de Barcelona

#     utm_x, utm_y = transform(proj_wgs84, proj_utm, x.Longitude, x.Latitude)
#     utm_lons, utm_lats = transform(proj_wgs84, proj_utm, lons, lats)

#     ok = OrdinaryKriging(utm_x, utm_y, x.NO2)
#     interpol_points, ss_cp = ok.execute('point', utm_lons, utm_lats)
...
#     lats, lons = np.meshgrid(lats, lons)
#     coords = np.vstack([lats.ravel(), lons.ravel()]).T
#     dist, idx = KDTree(all_points[:, :2]).query(coords)
